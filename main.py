            ...
            if not r.exists(f"user:{user_id}"):
                ...
                try:
                    context.bot.send_message(
                        chat_id=referrer_id,
                        text=f"🎉 Новый пользователь по вашей ссылке!\nПодписка продлена на {config['referral']['bonus_days']} дней."
                    )
                except:
                    pass
                
                update.message.reply_text(f"🎉 Пробный период на {config['referral']['trial_days']} дней!")
                return
...
def handle_callback(update: Update, context: CallbackContext):
    ...
    if query.data == 'get_trial':
        if not r.exists(f"user:{user_id}"):
            ...
            query.edit_message_text("✅ Ключ отправлен!")
        else:
            query.edit_message_text("⚠️ У вас уже есть подписка!")
        ...
        if r.exists(f"user:{user_id}"):
            ...
            if os.path.exists(file_path):
                with open(file_path, 'rb') as f:
                    query.bot.send_document(chat_id=user_id, document=f, caption="🔑 Ваш VPN конфиг")
                query.edit_message_text("✅ Конфиг отправлен!")
            else:
                query.edit_message_text("❌ Файл не найден")
        else:
            query.edit_message_text("❌ Нет активной подписки")
        ...
        try:
            ...
            query.edit_message_text("❌ Ошибка создания ключа")
...
def main():
    ...
    jq.run_repeating(check_expired_keys, interval=86400)
